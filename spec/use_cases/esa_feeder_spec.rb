...
RSpec.describe EsaFeeder::UseCases::Feed do
  describe 'tags not contain me_tags' do
    ...
    before do
      expect(esa_client).to receive(:find_templates) .with('feed_mon').once .and_return(mon_templates)
  ...
  describe 'tags contains me_tags' do
    ...
    let(:expected) do
      [
        { me_templates[0].number => me_posts[0].number },
        { me_templates[1].number => me_posts[1].number }
      ]
    end
    ...
    subject do
      described_class.new(esa_client, slack_client)
                     .call(%w[feed_wday], 'esa_bot')
    end

    it 'create some posts' do
      expect(subject).to eq(expected)
    end
  end
end
